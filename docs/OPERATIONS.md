...
The favicon, app icons, OG link-preview image, and web manifest are all
generated by one committed script from two sources: the words in
`frontend/site.config.json` and the colors in
`frontend/src/styles/global.css`. After any rebrand — new tagline, new
palette, a designed replacement for the plain mark — regenerate and
rebuild:
...
it, and it draws with the certificate DejaVu fonts already in the repo.
A designer replacing the mark entirely replaces the script's drawing
functions or just overwrites the generated files; nothing else in the
app references them except by filename.)
