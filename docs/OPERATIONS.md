...
2. `/srv/supercpe/repo/deploy/rollback.sh <sha>` — a checkout, a rebuild
   from cache (deploy.sh keeps the newest three deploys' images per repo
   and build cache from the last seven days; an older sha rebuilds from
   scratch), a restart, and the same health verification.
...
Every brand asset the site and the certificate show is derived from one
committed directory, `brand/` at the repo root — the files Dane produced
(`brand/README.md` maps each file to its role and records the sampled
palette). Nothing in the app reads `brand/` directly. One script derives
every fixed-name copy:

    backend/.venv/bin/python frontend/scripts/sync_brand.py
    backend/.venv/bin/python frontend/scripts/sync_brand.py --check
    cd frontend && npm run build

The first line writes the favicon set (`favicon.ico`, `favicon-16x16.png`,
`favicon-32x32.png`, `apple-touch-icon.png` — copied verbatim from
`brand/`), the manifest icons (`icon-192.png`, `icon-512.png`, the mark
on white, inset for a maskable crop), `logo.png` (the JSON-LD logo),
`og.png` (the logo and the one line from `site.config.json`), the
manifest, the hashed-pipeline copies under `frontend/src/assets/brand/`
(the site header, the landing page, the admin nav), and the backend's
copies under `backend/app/assets/brand/` (the certificate's logo and
seal, and `palette.py` — every `--color-*` token from `global.css`). The
backend never imports from `frontend/` and the api image copies
`backend/` only, which is why the copies exist. Every output is a pure
function of the inputs: a second run is byte-identical, and `--check`
exits non-zero naming any stale file. `--check` is on the pre-changelog
lint line in CLAUDE.md and runs in the test suite
(`test_committed_brand_assets_are_what_sync_brand_writes`), so a palette
edit or a new file in `brand/` without a script run fails locally.

The script runs on the backend venv's Python because Pillow ships with it
(a WeasyPrint dependency). The brand sources are PNG only, so no SVG
rasterizer is needed and there is no SVG favicon; if Dane ever supplies
SVGs, the script is where an SVG favicon and a vector certificate mark
would be added.

**The Flaticon icon is retired (033).** 024's `favicon.svg` was a
Flaticon-licensed graphic, permitted as a favicon and forbidden as a
logo; it was deleted with 033 and nothing references it. The 022/032
`generate_identity.py` (the "sC" monogram, the DejaVu-drawn OG card)
is gone with it. Every mark on every surface is now the brand's own,
owned outright.

**Palette.** `global.css` carries the brand tokens (`--color-brand-blue`
`#006afc`, `--color-brand-navy` `#012c6b`, `--color-brand-teal`
`#01b6af`) and `--color-accent` `#0066f4`, a tint of the brand blue that
clears WCAG AA as text on white and on the page ground where the true
blue does not; the teal is never used as text. A palette change is an
edit to `global.css`, a script run, and a commit.

When the browser keeps showing an old icon, bump the `?v=` query on the
icon links in `frontend/index.html` (033 set it to 3).
...
- **Favicons cache hard too.** The header and landing logo are
  content-hashed by the build, so a rebrand busts them; `favicon.ico`,
  the PNG favicons, `apple-touch-icon.png`, `logo.png`, and `og.png` keep
  fixed names (old browsers and scrapers fetch them blindly), so after a
  rebrand those change in place and simply take time.
...
Caddyfile, so shipping 022 needed the routine deploy plus a Caddy
...
and `certificate.css` by WeasyPrint, in the brand palette
(`backend/app/assets/brand/palette.py`, written by `sync_brand.py`
...
- **The mark.** Top of the page: the logo uploaded on `/admin/sponsor`
  ...
  `sponsor/logo.<ext>`), else the superCPE brand logo
  (`backend/app/assets/brand/logo.png`, 033; the brand mark also sits
  behind the award as a faint seal). The logo is presentation: it is
  not in the snapshot, and uploading or clearing it changes only
  certificates rendered from then on. The renderer fetches nothing but
  files under `backend/app/assets/` (the fonts and the brand images);
  the PDF's creation date is the snapshot's completion instant, so two
...
- **Issued certificates are never re-rendered.** A PDF stored at
  ...
  032 is `docs/certificate-sample.png`; 033 replaced the monogram with
  the brand logo and re-coloured the frame (`docs/certificate-sample-033.png`).
